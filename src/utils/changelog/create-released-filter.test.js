...
describe('createReleasedFilter', () => {
  ...
  it('should throw if no version passed', () => {
    ...
    expect(() =>
      createReleasedFilter(mockVersion, config)
    ).toThrowErrorMatchingInlineSnapshot(
      `"Cannot create released filter, \`version\` argument is missing."`
    )
  })
  ...
  it('should throw if no config passed', () => {
    ...
    expect(() =>
      createReleasedFilter(mockVersion, mockConfig)
    ).toThrowErrorMatchingInlineSnapshot(
      `"Cannot create released filter, \`config\` argument is missing."`
    )
  })
